...
async def broadcast_messages(user_id, message):
    try:
        ...
        logging.info(f"{user_id}-Removed From Database, Since Delete Account.")
...
async def send_all(bot, userid, files, ident):
    if AUTH_CHANNEL and not await is_subscribed(bot=bot, userid=userid):
        ...
        if ident == 'filep' or 'checksubp':
            ...
            pre = 'checksub' 
        btn = [[
                InlineKeyboardButton("❆ Join Our Update Channel ❆", url=invite_link.invite_link)
            ],[
                InlineKeyboardButton("↻ Tʀʏ Aɢᴀɪɴ", callback_data=f"{pre}#send_all")
            ]]
        await bot.send_message(
            chat_id=userid,
            text="**You are still not join our Update Channel.\n\n Join Now By Below Button to stay Update.**",
    ...
    if IS_VERIFY and not await check_verification(bot, userid):
        ...
        await bot.send_message(
            chat_id=userid,
            text="<b>You are not Verified!\nKindky Verify to access All Files untill 12 Hours. After 12 Hours, You can again Verify and take Benefits of this Bot Freely.</b>",
            ...
            await bot.send_cached_media(
                ...
                reply_markup=InlineKeyboardMarkup(
                    [
                        ...
                        InlineKeyboardButton("Bᴏᴛ Oᴡɴᴇʀ", url="t.me/Sandip10x")
                        ]
                    ]
                )
            )
